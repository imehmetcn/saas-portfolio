...
export default function ProfilePage() {
  const { profileData, heroData, updateProfileData, updateHeroData } = useAdmin();
  ...
  const handleSave = async (section: 'profile' | 'hero') => {
    setIsLoading(true);
    try {
      if (section === 'profile') {
        ...
        toast.success('Ana sayfa bilgileri başarıyla güncellendi! 🎉', {
          duration: 3000,
          position: 'top-right',
        });
      }
  ...
  const updateHeroForm = (field: string, value: string | string[]) => {
    setHeroForm(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex">
      ...
      <div className="flex-1 flex flex-col">
        <Header
          title="Profil & Ana Sayfa"
          description="Kişisel bilgilerinizi ve ana sayfa içeriğini düzenleyin"
        />
          ...
          {activeTab === 'hero' && (
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white">Ana Sayfa İçeriği</h2>
                {!isEditing ? (
                  <motion.button
                    onClick={() => setIsEditing(true)}
              ...
              {isEditing ? (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => updateHeroForm('description', e.target.value)}
                      rows={4}
                      className="w-full bg-slate-700/50 border border-slate-600/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all duration-300 resize-none"
                      placeholder="Kendinizi tanıtan açıklama..."
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      value={heroForm.skills.join(', ')}
                      onChange={(e) => updateHeroForm('skills', e.target.value.split(', ').filter(s => s.trim()))}
                      className="w-full bg-slate-700/50 border border-slate-600/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all duration-300"
                ...
                <div className="space-y-6">
                  <div>
                    <h3 className="text-lg font-semibold text-slate-300 mb-3">Ana Başlık</h3>
                    <p className="text-white text-xl font-bold">{heroForm.title}</p>
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold text-slate-300 mb-3">Alt Başlık</h3>
                    <p className="text-slate-400 text-lg">{heroForm.subtitle}</p>
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold text-slate-300 mb-3">Açıklama</h3>
                    <p className="text-slate-400 leading-relaxed">{heroForm.description}</p>
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold text-slate-300 mb-3">Yetenekler</h3>
                    <div className="flex flex-wrap gap-2">
                      {heroForm.skills.map((skill, index) => (
                        <span
                          key={index}
